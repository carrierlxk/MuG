...
def framepair_loader(data_dir, video_path, frame_start, frame_end):
	...
	for ii in range(2):
		...
		#success, image = cap.read()
		#print('im path:',id_[ii], os.path.join(data_dir, video_path,  '{:06d}.jpeg'.format(int(id_[ii]))))
		image = cv2.imread(os.path.join(data_dir, video_path,  '{:06d}.jpeg'.format(int(id_[ii]))))
...
class VidListv1(torch.utils.data.Dataset):
	...
	def __getitem__(self, idx):
		...
		pair_ = framepair_loader(self.data_dir,video_, 0, frame_end)
